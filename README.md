...
```
data/
  curated/idea_spaces.csv      # curated seeds you maintain (tracked, never cleaned)
  curated/dependencies.csv     #   canonical dependencies + their viability thresholds
  curated/cited_anchors.csv    #   hand-entered figures for paywalled/cited-only sources
  sources/<provider>/          # RAW provider files, immutable, committed:
                               #   owid/ irena/ bnef/ (metric data)
                               #   crunchbase/ startup-failures/ (enrichment)
  derived/metric_observations_full.csv   # GENERATED by build-metric-data — never hand-edit
  derived/capacity_series.csv            #   cumulative deployment, the Wright fit's x-axis
...
The split is by **authorship**: if a person maintains it, it's `curated/`; if a provider
produced it, it's `sources/`; if a script produces it, it's `derived/`. `sources/` is never
written by code and `derived/` is never written by hand.
...
and `output/` but leaves `curated/`, `sources/` and `derived/` intact.
...
$ npm run build-metric-data   # data/sources + data/curated -> data/derived
$ npm run build      # -> data/output/<run>/lazarus.db

$ npm run clean      # wipe scraped/ + output/, keep curated/ sources/ derived/
```
...
`data/sources/**` plus the hand-entered figures in `data/curated/**`. It makes no LLM or
network calls, so it's cheap and always safe to re-run. Its outputs are **committed**, and
...
**The audit loop requires [uv](https://docs.astral.sh/uv/).** One source is an IRENA
`.xlsb` binary workbook, read by `extract/irena_capacity.py`. Without uv the step **fails
loudly and writes nothing** rather than silently producing a short `capacity_series.csv` —
a skipped extractor after that `rm` would leave a phantom deletion in the very diff you're
reading as the audit's answer.

`--allow-stale-metric-data` is the one escape hatch: it reuses the committed
`capacity_series.csv` instead of regenerating it, and still fails if that file is absent.
It is named for what you accept, not what it disables. Reach for it only when you knowingly
want the other extractors refreshed on a machine without uv — never when reading a diff:

```
$ npm run build-metric-data -- --allow-stale-metric-data
```

Pipeline runs always take the strict path; the orchestrator passes no flags.
