...
// Human-authored inputs, edited in PRs. The name states the authorship rule that
// separates it from sourcesDir: if a person maintains it, it lives here.
export const curatedDir = join(config.dataDir, 'curated')
// Generated by build-metric-data from sourcesDir + curatedDir. Committed (the
// rebuild-and-diff audit needs them tracked) but never hand-edited.
export const derivedDir = join(config.dataDir, 'derived')
